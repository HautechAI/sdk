...
import { v4 } from 'uuid';
import { type CreatePipelineParamsDto, PipelineDtoStatus } from '../../autogenerated/schemas';
    ...
    describe('Pipeline Construction and Execution', () => {
        it('should construct a pipeline using the fluent API and run it', async () => {
            const pipelineData = sdk.pipelines.constructTemplate((pipeline) => {
                ...
                pipeline.setOutputRef(awaitedEcho2.result.output);

                return pipeline;
            });

            pipelineData.setInput({ message: 'Hello from constructed pipeline!' });
        ...
        it('should construct a pipeline with multiple tasks and dependencies', async () => {
            const pipeline = sdk.pipelines.constructTemplate((pipeline) => {
                // First task
